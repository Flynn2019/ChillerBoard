...
vu16 oldValue[ADC1_Count][ADC_ChannelCount];
vu16 ADC_Filter[ADC_ChannelCount];   					//AD滤波后,计算一个通道ADC1_Count次数据的均值
u32 TMonitor[3]={0};									//存放检测所得的温度值，依次为低温,高温,环境温度
...
char Tflag=0;
int TemperaError[3]={0};								//温度补偿误差值有符号
int TemperaErrorDis[3]={100,100,100};								//温度补偿误差值显变量
...
void ADC1_Tempera(void)
{
	...
	for(i=0;i<ADC_ChannelCount;i++)
	{
		...
		*/
		...
		{
			if(((ADGetValue[k][i]-oldValue[k][i])>ValueCorrection)||((oldValue[k][i]-ADGetValue[k][i])>ValueCorrection))
			{
				oldValue[k][i]=ADGetValue[k][i];
			}
			ValueCount+=oldValue[k][i];
	...
	*/
	if(Key.MenuSelect==5)
	{
		ADC_SmoothCount[1]=0x05C0;
		ADC_SmoothCount[3]=0x05BC;
		ADC_SmoothCount[5]=0x05BD;
	}
	for(m=0;m<3;m++)
	{
		TMonitor[m]=(((2361*ADC_SmoothCount[2*m])/(ADC_SmoothCount[2*m+1])-2597)+TemperaError[m]);//已经扩大10倍的值转化为温度数据,低温,高温,环境温度
		/**
		  **设置温度最大计数值和最小计数值
		  **使温度不至于太大方便后面变量TMonitorDis[m]得恢复
		*/
		if(TMonitor[m]>=(TemperaSet[3]+5))
		{
			TMonitor[m]=(TemperaSet[3]+5);
		}
		if(TMonitor[m]<=15)
		{
			TMonitor[m]=15;
		...
		*@稳定数值显示,去除0.1度浮动显示
		*/
		if((TMonitorDis[m]<(TMonitor[m]-2))||(TMonitorDis[m]>(TMonitor[m]+2)))
		{
			/*
			*@显示处理缓慢变化
			*/
	...
	*/
	if(TMonitorDis[0]>=100)//在防冻温度之外
	{
		...
		*/
		...
		if((TMonitorDis[2]<30)||(TMonitorDis[2]>TemperaSet[3]))//超出工作的环境温度范围3~45度
		{
			/*开启环境温度异常标志*/
			Alarm.Environ=1;
			Alarm.flag=1;
		...
		*
		*/
		if(TMonitorDis[0]>(TemperaSet[0]-(TemperaSet[4]/2)))//低温水水温高于设定值，开启压缩机
		{
			/*关闭电加热器*/
			Chiller.ElBow_ON=0;
			Chiller.ElBow_OFF=1;
		...
		if(TMonitorDis[0]<(TemperaSet[0]-(TemperaSet[4]/2)))//低温水水正常后关闭压缩机风机
		{
			/*关闭压缩机风机*/
			Chiller.Compressor_ON=0;
			Chiller.Compressor_OFF=1;
			if(TMonitorDis[0]<(TemperaSet[0]-(TemperaSet[4]/2)-20))
			{
				/*开启电加热器*/
				Chiller.ElBow_ON=1;
				Chiller.ElBow_OFF=0;
		...
		**高温水温高于设定值，开启水阀
		*/
		if((TMonitorDis[0]>(TMonitorDis[1]+20))|| \
			(TMonitorDis[1]>(TemperaSet[1]+(TemperaSet[5]/2))))
		{
			/*开启水阀*/
			Chiller.Valve_ON=1;
			Chiller.Valve_OFF=0;
		...
		**高温水水温正常后关闭水阀
		*/
		if((TMonitorDis[0]<(TMonitorDis[1]+10))&& \
			(TMonitorDis[1]<(TemperaSet[1]-(TemperaSet[5]/2))))
		{
			/*关闭水阀*/
			Chiller.Valve_ON=0;
