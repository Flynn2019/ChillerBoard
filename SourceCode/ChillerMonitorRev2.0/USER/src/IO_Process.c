...
static u8 CompressorDelayFlag =0;//启动压缩机延时标志位
/**
  * @brief	普通输入输出引脚配置
  * @param None
  * @retval : None
...
void InputMonitor(void){
	...
	else//一个时候只显示一个警告
	{
		...
		((Alarm.Environ+Alarm.LowerWaterSet+Alarm.UpperWaterSet)!=0))
		{
			...
			else if(GPIO_ReadInputDataBit(InPinCtrl_L,Powerphase_IN) == Bit_SET){//相序异常
				/*异常显示标识开启*/
				Alarm.Powerphase=1;
				/*禁止所有外设工作*/
				Chiller.Pump_OFF=1;
			...
			else if(GPIO_ReadInputDataBit(InPinCtrl_L,Compressor_IN) == Bit_SET){//压缩机过载
				Alarm.Compressor=1;
				/*关闭压缩机*/
				Chiller.Compressor_OFF=1;
				Chiller.Compressor_ON=0;
			...
			else if(GPIO_ReadInputDataBit(InPinCtrl_L,Level_IN) == Bit_SET){//液位开关异常
				Alarm.Level=1;
				/*关闭水泵,压缩机*/
				Chiller.Pump_OFF=1;
				Chiller.Pump_ON=0;
			...
			else if(GPIO_ReadInputDataBit(InPinCtrl_L,HighWater_IN) == Bit_SET){//高温水流量异常
			...
			else if(GPIO_ReadInputDataBit(InPinCtrl_H,LowWater_IN) == Bit_SET){//低温水流量异常
			...
			else if(GPIO_ReadInputDataBit(InPinCtrl_H,Pumphigh_IN) == Bit_SET){//高温水泵过载
				Alarm.Pumphigh=1;
				/*关闭水泵*/
				Chiller.Pump_OFF=1;
				Chiller.Pump_ON=0;
			...
			else if(GPIO_ReadInputDataBit(InPinCtrl_H,Pumplow_IN) == Bit_SET){//低温水泵过载
				Alarm.Pumplow=1;
				/*关闭水泵,压缩机*/
				Chiller.Pump_OFF=1;
				Chiller.Pump_ON=0;
...
void OutPro(void){//处理外设输出
	InputMonitor();//获得输入状态
	/**********************************************************************************
	*压缩机关闭后到下次开启至少延迟大小为CompressorDelayTime的时间
	**********************************************************************************/
	if(Chiller.Compressor_OFF==1)//压缩机关了之后延时1分钟
	{
		if(CompressorDelayFlag==0)//压缩机延时标志启动一次后有效
		{
			CompressorTimeCount=0;//每次关机时启动压缩机延时
		}
	...
	if(Chiller.Compressor_ON==1)
	{
		...
		if(CompressorTimeCount>=CompressorDelayTime)
		{
			Chiller.Compressor_ON=1;		//延迟时间到达后恢复输出
		}
	}
	/**********************************************************************************/
