...
Menu_Typedef CH_SetupMenuItems[MenuNum] = 
{
	...
	{"低温水对比温度差","差值温度：    ℃","按  键设置温度  ","                ",EnterTemperaSetting},
	{"高温水对比温度差","差值温度：    ℃","按  键设置温度  ","                ",EnterTemperaSetting},
	/*
	*以下为非开发人员不显示界面,不对使用用户开放
	*/
	{"低温水温度校正值","校正温度：    ℃","校正参数：    ℃","按  键设置温度  ",EnterTemperaSetting},
	{"高温水温度校正值","校正温度：    ℃","校正参数：    ℃","按  键设置温度  ",EnterTemperaSetting},
	{"环境温度校正值  ","校正温度：    ℃","校正参数：    ℃","按  键设置温度  ",EnterTemperaSetting}
...
{
	if(Key.MenuSelect>=(MenuNum-3))
	{
		LCD_Setpos_DispDecimal(2,6,TMonitor[Key.MenuSelect-(MenuNum-3)],1);//显示温度
		LCD_Setpos_DispDecimal(3,6,TemperaErrorDis[Key.MenuSelect-(MenuNum-3)],1);//显示温度
		LCD_Setpos(4,2);
	...
	{
		LCD_Setpos_DispDecimal(2,6,TemperaSet[Key.MenuSelect-7],1);//显示温度
		LCD_Setpos(3,2);
		LCD_WriteByte(0x1e);//上键图标
		LCD_WriteByte(0x1f);//下键图标
	}
...
void AlarmDis(void){
	if(Alarm.flag>0)
	{
		...
		else if(Alarm.Compressor==1)								//压缩机过载异常
			LCD_Setpos_DispString(2,1,*(pWarning+1));
		else if(Alarm.Level==1)									//液位过低异常
			LCD_Setpos_DispString(2,1,*(pWarning+2));
		else if(Alarm.HighWaterFlow==1)							//高温水流量异常
			LCD_Setpos_DispString(2,1,*(pWarning+3));
		else if(Alarm.LowWaterFlow==1)							//低温水流量异常
			LCD_Setpos_DispString(2,1,*(pWarning+4));
		else if(Alarm.Pumphigh==1)								//高温水泵过载异常
			LCD_Setpos_DispString(2,1,*(pWarning+5));
		else if(Alarm.Pumplow==1)								//低温水泵过载异常
...
void Dis_Menu(char MenuDis)
{
	...
	pCurrentMenu=(CH_SetupMenuItems+MenuDis);
